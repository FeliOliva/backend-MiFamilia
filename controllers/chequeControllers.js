...
const clearChequeCache = async () => {
    try {
        const keys = await redisClient.keys("Cheques:*");
        const negocioKeys = await redisClient.keys("ChequesNegocio:*");
        ...
        console.error("Error al limpiar la caché de Cheques:", error);
    }
};
...
const getCheques = async (req, res) => {
    try {
        ...
        const cacheKey = `Cheques:${limitNumber}:${pageNumber}`;
        ...
        const chequesData = await chequeModel.getAllCheques(limitNumber, pageNumber);
        await redisClient.setEx(cacheKey, 600, JSON.stringify(chequesData));
...
const getChequesByNegocio = async (req, res) => {
    try {
        ...
        const cacheKey = `ChequesNegocio:${id}`;
        const cachedData = await redisClient.get(cacheKey);
        ...
        const chequesData = await chequeModel.getChequesByNegocio(id, limitNumber, pageNumber);
        await redisClient.setEx(cacheKey, 600, JSON.stringify(chequesData));
...
const addCheque = async (req, res) => {
    try {
        const { banco, nroCheque, fechaEmision, fechaCobro, monto, negocioId } = req.body;

        // Verificar que todos los campos estén presentes
        if (!banco || !nroCheque || !fechaEmision || !fechaCobro || !monto || !negocioId) {
...
const updateCheque = async (req, res) => {
    try {
        ...
        await clearChequeCache();
        const updatedCheque = await chequeModel.updateCheque(id, { banco: banco.toUpperCase(), nroCheque, fechaEmision: parseDate(fechaEmision), fechaCobro: parseDate(fechaCobro), monto: parseInt(monto), negocioId: parseInt(negocioId) });
